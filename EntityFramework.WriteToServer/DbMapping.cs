...
namespace EntityFramework.WriteToServer
{
    internal class DbMapping
    {
            ...
            foreach (var entitySetMap in entitySetMaps)
            {
                ...
                foreach (var typeMapping in typeMappings)
                {
                    ...
                    foreach (EntityType type in types)
                    {
                        ...
                        var properties = new List<EdmMember>(type.Properties);
        ...
        {
            var identity = tableEdmType.FullName;
        ...
        private void MapColumn(string identity, TableMapping tableMapping, EdmProperty property)
        {
            ...
            var edmMember = _tableColumnEdmMembers[identity].Single(c => c.Name == columnName);

            var propertyName = edmMember.Name;

            var columnMapping = tableMapping.AddColumn(propertyName, columnName);

            BuildColumnMapping(identity, property, propertyName, columnMapping);
        }

        private void BuildColumnMapping(string identity, EdmMember property, string propertyName, ColumnMapping columnMapping)
        {
